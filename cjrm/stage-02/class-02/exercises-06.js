...
if (age <= 7 || age >= 65) {
  ticket += 'Para você, a entrada é grátis!'
} else {
  ticket += 'Para você, a entrada é R$ 30,00.'
}
...
let numbersCrazyArray = []
let stringsCrazyArray = []
let booleansCrazyArray = []

for (let i = 0; i < crazyArray.length; i++) {
  if (typeof crazyArray[i] === 'number') {
    numbersCrazyArray.push(crazyArray[i])
  }
  if (typeof crazyArray[i] === 'string') {
    stringsCrazyArray.push(crazyArray[i])
  }
  if(typeof crazyArray[i] === 'boolean') {
    booleansCrazyArray.push(crazyArray[i])
  }
}

console.log(`O crazyArray tem ${booleansCrazyArray.length} booleans, ${numbersCrazyArray.length} números e ${stringsCrazyArray.length} strings.`)
...
const pairNumbersString = pairNumbers.join(', ').replace(', 5', ' e 5')

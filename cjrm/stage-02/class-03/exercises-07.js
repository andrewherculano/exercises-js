...
for (let i = 0; i < randomNumbers.length; i++) {
  ...
  const numbersGreatherThan400 = sumNumbers > 400

  if (numbersGreatherThan400) {
...
for (let i = 0; randomValues.length; i++) {
  let typeOfStrings = typeof randomValues[i] === 'string'
  let typeOfBooleans = typeof randomValues[i] === 'boolean'
...
let messageStrings = `${fourStrings.join(', ').replace(`, ${fourStrings[fourStrings.length -1]}`, ` e ${fourStrings[fourStrings.length -1]}`)}`

console.log(`3 informações sobre o array randomValues:
  - As primeiras 4 strings são ${messageStrings};
  - Até que as primeiras 4 strings fossem iteradas, ${countBoolean} booleans foram iterados;
  - O array foi iterado por ${countInteractions} vezes.`)

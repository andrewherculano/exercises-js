...
const getUsersPlaceHolder = (url) => new Promise((resolve, reject) => {
...
getUsersPlaceHolder('https://jsonplaceholder.typicode.com/users')
...
const getOperationMessage = (numberOne, operator, numberTwo, operation) => {
  return `${numberOne} ${operator} ${numberTwo} = ${operation}`
}

const calculator = (operator) => (numberOne, numberTwo) => {
  const operations = {
    '+': getOperationMessage(numberOne, operator, numberTwo, numberOne + numberTwo),
    '-': getOperationMessage(numberOne, operator, numberTwo, numberOne - numberTwo),
    '*': getOperationMessage(numberOne, operator, numberTwo, numberOne * numberTwo),
    '/': getOperationMessage(numberOne, operator, numberTwo, numberOne / numberTwo),
    '%': getOperationMessage(numberOne, operator, numberTwo, numberOne % numberTwo)
...
const newSudeste = brasil.splice(5, 7)
...
const filterIds = newNewBrasil.filter(({ id }) => id % 2 === 0)

...
const x = (callback) => callback("Andrew");
x(logGreeting);
...
const filterNumbers = (num) => num < 5;

const numbers = [3, 4, 10, 20];
const lesserThanFive = numbers.filter(filterNumbers);
...
const sumPrices = (accumulator, item) => accumulator + item;
...
const myFunc = (param1, param2, param3) => {
  const checkParametersUndefined = [param1, param2, param3].includes(undefined);
  return checkParametersUndefined
...
const getAvaliabeSpaces = (spaces, booksIn) => {
...
booksBox.addBooks = (booksQuantity) => {
  ...
  const boxSpacesAreNotEnouth = booksBox.booksIn + booksQuantity > spaces

  if (isBoxFilled) {
    return 'A caixa já está cheia'
  }

  if (boxSpacesAreNotEnouth) {
    return getAvaliabeSpaces(spaces, booksBox.booksIn)
  }

...
const getPokemonsFromApi = (url, callback) => {
...
getPokemonsFromApi(bulbasaur, (error, data) => {
  logPokemon(error, data)

  getPokemonsFromApi(charmander, (error, data) => {
    logPokemon(error, data)

    getPokemonsFromApi(squirtle, (error, data) => {
      logPokemon(error, data)
    })
  })
})
...
const arrayColors = ['black', 'blue', 'pink', 'red', 'yellow', 'brown', 'orange', 'purple']

const logMessageColors = (item) => {
  console.log(convertToHex(item))
}

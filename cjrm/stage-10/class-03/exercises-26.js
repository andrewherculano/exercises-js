...
const formatDate = (date) => {
  ...
  return `${formatUnit(day)}/${formatUnit(month)}/${year}`;
};
...
const formatDateInfo = (date) => {
  const hour = formatUnit(date.getHours());
  const minute = formatUnit(date.getMinutes());

  const dayWeek = daysWeek[date.getDay()];
  const dayMonth = formatUnit(date.getDate());

  const month = months[date.getMonth()];
  const year = formatUnit(date.getFullYear());

  return `${hour}:${minute} - ${dayWeek}, ${dayMonth} de ${month} de ${year}`;
};
...
const updateClock = () => {
  const present = new Date();
  const hours = formatUnit(present.getHours());
  const minutes = formatUnit(present.getMinutes());
  const seconds = formatUnit(present.getSeconds());
